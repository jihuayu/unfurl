...
- Image proxy with:
  ...
  - `/api` image/logo URLs rewritten to local proxy URLs with source page `referer`
  - background warmup for `og:image` and page icons on metadata fetch
  - optional SQLite-backed processed image cache
...
Cache behavior:
- when `CACHE_BACKEND=none`, OG metadata is not cached locally
...
docker run --rm -p 8080:8080 unfurl-server
```
...
- Metadata cache uses the configured `CACHE_BACKEND` and defaults to SQLite
...
- SQLite metadata/image pools reduced to `1` when SQLite backends are enabled
- shorter SQLite idle timeout when SQLite backends are enabled
...
- `none` mode: no app-local cache persistence
...
- image and logo URLs generated by `/api` are local proxy URLs and include `referer=<source page url>`
